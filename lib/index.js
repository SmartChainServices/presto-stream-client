...
function makeRequest(opts, contentBody, sslOptions, jsonParser, callback) {
  ...
  new Promise((resolve, reject) => {
    ...
    (response) => {
      if (response.response_code === 503 || response.response_code === 404) {
        // https://github.com/prestodb/presto/wiki/HTTP-Protocol "If the response is an HTTP 503, sleep 50-100ms and try again"
        setTimeout(() => {
          return makeRequest(
            opts,
            contentBody,
        ...
        if (
          ...
          //some apis such as DELETE do not have a body
        ...
        try {
          ...
          return callback(
            new prestoError(
              "request failed: unintelligible response.",
              response
            )
          );
        }
...
class Client {
  ...
  constructor(args = {}) {
    ...
    this.objectMode =
      args.hasOwnProperty("objectMode") && args.objectMode ? true : false;
    this[s_options].headers[Headers.USER_AGENT] =
      "presto-stream-client-" + VERSION;
    ...
    if (args.password) {
      this[s_options].headers[Headers.AUTHORIZATION] =
        "Basic " +
        Buffer.from(
          this[s_options].headers[Headers.USER] + ":" + args.password
        ).toString("base64");
    }
    ...
    this[s_request] = (opts, callback) => {
      ...
      if (opts.headers) {
        //headers is child object, copy by value, override if provided
   ...
   */
  async cluster() {
    const { data } = await this[s_requestPromise]({
      method: "GET",
      path: "/v1/cluster",
    });
    return data;
  ...
  async execute(opts) {
    ...
    const { response_code, data, session } = await this[s_requestPromise]({
      method: "POST",
      path: "/v1/statement",
      headers: header,
      body: opts.query,
    ...
    if (response_code !== 200 || (data && data.error)) {
      if (data.error.message) {
        ...
        throw new prestoError(
          "execution error" + (data && data.length > 0 ? ":" + data : ""),
          { response_code }
        );
      }
    } else if (!data.id) {
    ...
    const streamOpts = {};
    streamOpts.objectMode = opts.hasOwnProperty("objectMode")
      ? opts.objectMode
      : this.objectMode;
    if (opts.highWaterMark) {
      streamOpts.highWaterMark = opts.highWaterMark;
    }
    /*
    var data = {
      "stats": {
...
class Statement extends Readable {
  ...
  constructor(
    ...
    queryId,
    fetchInfo = false,
    pollInterval = QUERY_STATE_CHECK_INTERVAL,
    session = null
  ) {
    ...
    if (
      readableOptions.hasOwnProperty("objectMode") &&
      readableOptions.objectMode
    ) {
    ...
    this[s_handleError] = (err) => {
      ...
      this.cancel().catch((error) => {
        // if call to cancel errors, emit that error
  ...
  _statementCancelled() {
    if (this[s_cancelled]) {
      //check if cancelled before doing anything else.
      if (!this[s_EOF]) {
        //if not yet reached EOF push null to signal EOF
        this[s_EOF] = true;
        this.push(null);
      }
  ...
  _run() {
    ...
    this.client[s_request](requestOpts, (err, response) => {
      if (err) {
        ...
        return this[s_handleError](
          new prestoError("strange response from Presto", response)
        );
      }
      ...
      if (data.error) {
        if (data.error.message) {
          return this[s_handleError](
            new prestoError(data.error.message, { response_code, data })
          );
        } else {
          return this[s_handleError](
            new prestoError("attempt to retrieve next dataUri failed", {
              response_code,
              data,
            })
          );
      ...
      if (this.listenerCount("state") > 0 && !this[s_SOF]) {
        //only emit state event if something is listening & file has not started downloading
      ...
      if (!this.columns && data.columns) {
        ...
        if (this[s_objectMode]) {
          //object mode, guarantee uniqueness of column names
          ...
          if (Object.keys(duplicates).length > 0) {
            //only run if duplicates detected
          ...
          //not object mode, push out CSV header
          canPush = this.push(
            this.columns
              .map((i) => convertToCSV(i.name, this.client.jsonParser))
              .join(",") + "\n"
          );
        }
      }
      if (data.data) {
        if (this[s_objectMode]) {
          //if object mode, push out object in form column name: value
          ...
          //when data is being sent in string mode, send the entire response data as a string in one push
          canPush = this.push(
            data.data
              .map((i) =>
                i.map((y) => convertToCSV(y, this.client.jsonParser)).join(",")
              )
              .join("\n") + "\n"
          );
        }
      ...
      if (data.nextUri) {
        ...
        if (this.fetchInfo && data.infoUri) {
          ...
          this.client[s_request]({ path: data.infoUri }).then(
            ({ data: info }) => {
